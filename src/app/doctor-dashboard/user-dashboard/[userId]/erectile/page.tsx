...
import { Loader2, HeartPulse, Save, Trash2, Edit, XCircle } from 'lucide-react';
...
export default function UserErectileManagementPage({ params }: { params: Promise<{ userId: string }> }) {
    const {
        viewedUserData,
        loadingViewedUser,
        loadViewedUserData,
    ...
    const handleDeleteClick = async (log: ErectileLogEntry) => {
        ...
        try {
            await deleteErectileLog(userId, log.id);
            toast({
    ...
    const onSubmit: SubmitHandler<ErectileFormInputs> = async (data) => {
        ...
        if (!viewedUserData) {
            ...
            setIsSubmitting(false); // Garante que o botão de submit seja reativado
            return;
        }
        ...
        try {
            if (editingLogId) {
                ...
                const existingEntry = viewedUserData.erectileLogs.find(log =>
    ...
    return (
        <>
            <PageHeader
                title={`Gerenciar Função Erétil de ${viewedUserProfile?.displayName || 'Usuário'}`}
                description="Adicione, edite ou remova registros de função erétil para este usuário."
                icon={HeartPulse}
            />
